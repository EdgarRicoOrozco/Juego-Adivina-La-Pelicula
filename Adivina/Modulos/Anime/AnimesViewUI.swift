...
protocol AnimesViewUIDelegate {
    func goToBack()
}

class AnimesViewUI: UIView{
    ...
    private lazy var titleIcon: AnimationView = {
        let animation = Animation.named("anime",
                                        subdirectory: nil,
                                        animationCache: nil)
    ...
    var delegate: AnimesViewUIDelegate?
    var navigationController: UINavigationController?
    
    public convenience init(
        navigation: UINavigationController,
        delegate: AnimesViewUIDelegate){
        self.init()
    ...
    fileprivate func setupUIElements(){
        ...
        let cardViewElements = [marcaLabel,backButton,imageMovie,titleIcon,succesIcon,errorIcon,titleLabel,oneBtn,twoBtn,threeBtn,newBtn]
    ...
    @objc func new(){
        let preguntas : Int = AnimesBase.Preguntas.count
        AnimesBase.contador = AnimesBase.contador + 1
        if preguntas == AnimesBase.contador {
            AnimesBase.contador = 0
        }
        clean()
        setTitles()
    ...
    func setTitles(){
        imageMovie.image = UIImage(named: "A\(AnimesBase.contador)")
        oneBtn.setTitle(AnimesBase.Preguntas[AnimesBase.contador].cero, for: .normal)
        twoBtn.setTitle(AnimesBase.Preguntas[AnimesBase.contador].uno, for: .normal)
        threeBtn.setTitle(AnimesBase.Preguntas[AnimesBase.contador].dos, for: .normal)
    }
    ...
    @objc func response(_ sender: UIButton){
        if sender.tag == AnimesBase.Respuestas[AnimesBase.contador]{
            succes()
            matchBtns(tag: AnimesBase.Respuestas[AnimesBase.contador])
        }else{
            matchBtns(tag: AnimesBase.Respuestas[AnimesBase.contador])
            error()
        }
