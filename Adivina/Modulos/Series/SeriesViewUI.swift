...
class SeriesViewUI: UIView{
    ...
    private lazy var titleIcon: AnimationView = {
        let animation = Animation.named("netflix",
                                        subdirectory: nil,
                                        animationCache: nil)
    ...
    @objc func new(){
        let preguntas : Int = seriesBase.Preguntas.count
        seriesBase.contador = seriesBase.contador + 1
        if preguntas == seriesBase.contador {
            seriesBase.contador = 0
        }
        clean()
        setTitles()
    ...
    func setTitles(){
        imageMovie.image = UIImage(named: "S\(seriesBase.contador)")
        oneBtn.setTitle(seriesBase.Preguntas[seriesBase.contador].cero, for: .normal)
        twoBtn.setTitle(seriesBase.Preguntas[seriesBase.contador].uno, for: .normal)
        threeBtn.setTitle(seriesBase.Preguntas[seriesBase.contador].dos, for: .normal)
    }
    ...
    @objc func response(_ sender: UIButton){
        if sender.tag == seriesBase.Respuestas[seriesBase.contador]{
            succes()
            matchBtns(tag: seriesBase.Respuestas[seriesBase.contador])
        }else{
            matchBtns(tag: seriesBase.Respuestas[seriesBase.contador])
            error()
        }
